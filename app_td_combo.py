...
for mag_ticker in mags_tickers:
    with open(Path(DATA_DIR) / 'MAGs TDST Risk Data.xlsx', 'rb') as file:
        mag_df = pd.read_excel(file,sheet_name=mag_ticker)[::-1]
...
export_csv = mags_td_combo_complete_stitches['AMZN']
